...
class grocery_CRUD_Generic_Model  extends grocery_CRUD_Model  {
    ...
    function get_relation_n_n_selection_array($primary_key_value, $field_info)
    {
    	...
    	die(var_dump("{$field_info->relation_table}.{$field_info->primary_key_alias_to_selection_table} = {$field_info->selection_table}.{$selection_primary_key}"));
    ...
    function build_relation_n_n_subquery($field, $selection_table, $relation_table, $primary_key_alias_to_selection_table, $primary_key_selection_table, $primary_key_alias_to_this_table, $field_name, $this_table_primary_key = ""){
        return "(SELECT array_agg(DISTINCT ".$this->protect_identifiers($field).") FROM ".$this->protect_identifiers($selection_table)
